from django.contrib import admin
from django.db.models import ForeignKey


class CustomModelAdminMixin(admin.ModelAdmin):
    """
    Instead of declaring all the fields you want to expose concerning your model on Django Admin,
    you can use this fixture. It adds `list_display`, `readonly_fields`, and `raw_id_fields` for you automatically.
    """

    def __init__(self, model, admin_site):
        if self.list_display and self.list_display[0] == "__str__":
            self.list_display = [field.name for field in model._meta.fields if field.name != "id"]
        self.list_filter = tuple(set(["created_at", "updated_at"] + (list(self.list_filter) or [])))
        self.readonly_fields = tuple(set(["created_at", "updated_at"] + (list(self.readonly_fields) or [])))
        if not self.raw_id_fields:
            # Only for FOREIGN KEY fields
            raw_id_fields = []
            for key, value in model._meta._forward_fields_map.items():
                if type(value) is ForeignKey and not key.endswith("id"):
                    raw_id_fields.append(key)
            if raw_id_fields:
                self.raw_id_fields = raw_id_fields
                # Exclusively for this application.
                hardcoded_mapping_autocomplete_and_search_fields = {
                    "psychologist": ["psychologist__name"],
                    "client": ["client__name"],
                }
                for (
                    key,
                    value,
                ) in hardcoded_mapping_autocomplete_and_search_fields.items():
                    if key in self.raw_id_fields:
                        self.search_fields = tuple(set(value + (list(self.search_fields) or [])))
                        self.autocomplete_fields = tuple(set([key] + (list(self.autocomplete_fields) or [])))
        super(CustomModelAdminMixin, self).__init__(model, admin_site)
